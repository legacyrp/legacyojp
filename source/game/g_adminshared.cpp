...
qboolean M_isAdmin(gentity_t *ent){
	if ( ent->client->sess.loggedinadmin ){
		return qtrue;
	}
	return qfalse;
}
...
Returns true if they are a knight.
============
*/
qboolean M_isKnight(gentity_t *ent){
	if ( ent->client->sess.loggedinknight ){
		return qtrue;
	}
	return qfalse;
}
	...
	for( i = 0; i < numPassThroughElements; i++ ){
		...
		if ( Q_stricmp(cmd, passthroughfuncs[i].clientcommand) == 0 ){// Found Admin Command
			...
			if( !M_isAdmin(ent) && !M_isKnight(ent)){
				trap_Cvar_VariableStringBuffer( "g_mNotLoggedInAdmin", NotAdminString, sizeof(NotAdminString) );
