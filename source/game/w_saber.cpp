...
{//returns the DP cost to block this attack for this attacker/defender combo.
	...
	{//standard bolt block!
			...
			|| attacker->activator && attacker->activator->s.weapon == WP_FLECHETTE)
		{
			...
			{
				if(defender->client->ps.fd.saberAnimLevel != SS_MEDIUM)
					saberBlockCost++;
...
if(WalkCheck(defender) && defender->client->ps.weapon != WP_SABER)
saberBlockCost+=1;
else if(defender->client->ps.weapon != WP_SABER);
saberBlockCost+=5;
...
{//have self go into the given dodgeAnim				   
	...
	//self->client->ps.weaponTime = 100; // was 300
...
qboolean G_DoDodge( gentity_t *self, gentity_t *shooter, vec3_t dmgOrigin, int hitLoc, int * dmg, int mod )
{
	...
	if(mod == MOD_SABER && shooter->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE]==FORCE_LEVEL_2)
		dpcost+=3;
	...
	//[/SaberSys]
	{
		...
		{//we hit an entity
			...
			if(hitEnt->client && hitEnt->client->pers.cmd.buttons & BUTTON_15)
			{
				...
				G_SetAnim(hitEnt, &hitEnt->client->pers.cmd, SETANIM_BOTH, BOTH_SWEEPKICK, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
...
static void G_KickSomeMofos(gentity_t *ent)
{
	...
	int	  kickPush = flrand( 10.0f, 20.0f );
	qboolean doKick = qfalse;
	renderInfo_t *ri = &ent->client->renderInfo;

	//BG_ForcePowerDrain(ent->client->ps,FP_SEE,7);
