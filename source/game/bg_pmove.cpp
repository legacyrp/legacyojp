...
int forcePowerNeeded[NUM_FORCE_POWER_LEVELS][NUM_FORCE_POWERS] = 
{
	...
	},
	{
		...
		//[/ForceSys]
		50,//FP_RAGE,//duration
		0,//FP_MANIPULATE,//duration -- 1.3 was 3
		50,//FP_ABSORB,//duration
		50,//FP_TEAM_HEAL,//instant
		20,//FP_LIFT,//instant -- 1.3 was 50
		//[ForceSys]
	...
	},
	{
		...
		//[/ForceSys]
		50,//FP_RAGE,//duration
		0,//FP_MANIPULATE,//duration -- 1.3 was 2
		25,//FP_ABSORB,//duration
		33,//FP_TEAM_HEAL,//instant
		20,//FP_LIFT,//instant -- 1.3 was 33
		//[ForceSys]
		//drain now acts like lightning.
		10,//FP_DRAIN,//initial FP cost
		//20,//FP_DRAIN,//hold/duration
...
extern void ST_FallToDeath(gentity_t*ent);
...
void PM_SetVelocityforLedgeMove( playerState_t *ps, int anim )
{
	...
	switch(anim)
	{
		...
		case BOTH_LEDGE_MERCPULL:
			...
			else if(animationpoint > .55 && animationpoint < .79 )
			{
				VectorSet(fwdAngles, 0, pm->ps->viewangles[YAW], 0);
				AngleVectors( fwdAngles, moveDir, NULL, NULL );
				//VectorScale(moveDir, 140, moveDir);
				VectorScale(moveDir, 110, moveDir);
				VectorCopy(moveDir, ps->velocity);
			}
			else
			{
			...
			/*else if( animationpoint > .14 && animationpoint < .150 )
			{
				ps->velocity[0] = 0;
				ps->velocity[1] = 0;
				ps->velocity[2] = 26;
			...
			else if( animationpoint > .675 && animationpoint < .7)
			{
				ps->velocity[0] = 0;
				ps->velocity[1] = 0;
				ps->velocity[2] = 140;
...
int NumberOfWeapons(void)
{
	...
	if(pm->ps->stats[STAT_WEAPONS] & (1 << WP_THERMAL))
		numWeaps++;
	if(pm->ps->stats[STAT_WEAPONS] & (1 << WP_GRENADE))
		numWeaps++;
...
static void PM_GroundTrace( void ) {
	...
	if ( trace.plane.normal[2] < minNormal ) {
		...
		//Com_Printf("%f < %f\n",trace.plane.normal[2],minNormal);
		pm->ps->groundEntityNum = ENTITYNUM_NONE;
		pml.groundPlane = qtrue;
		pml.walking = qfalse;
		return;
...
void PM_RocketLock( float lockDist, qboolean vehicleLock )
{
	...
	if((pm->ps->velocity[0] != 0 || pm->ps->velocity[1] != 0 || pm->ps->velocity[2] != 0) ||
		!BG_CrouchAnim(pm->ps->legsAnim))
		return;
...
qboolean CanDrainAmmo()
{//1.3
	if(pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] == -1)
		return qfalse;
	if(pm->ps->weapon == WP_BOWCASTER && (pm->ps->eFlags2 & EF2_BOWCASTERSCOPE) && (pm->cmd.buttons & BUTTON_ALT_ATTACK))
		return qfalse;
	if(pm->ps->weapon == WP_GRENADE && (pm->cmd.buttons & BUTTON_ALT_ATTACK))
		return qfalse;

	return qtrue;
}

qboolean CanFireWeapon()
{
#ifdef QAGAME
	gentity_t * ent = &g_entities[pm->ps->clientNum];
#endif
	 if(pm->ps->weapon == WP_ROCKET_LAUNCHER && !CanShootRocket())
	 {
#ifdef QAGAME
...
//1.3
	...
	{
		gentity_t * ent = &g_entities[pm->ps->clientNum];
		if(ent->client->attackNeedRelease && ! (pm->cmd.buttons & (BUTTON_ATTACK|BUTTON_ALT_ATTACK)))
			ent->client->attackNeedRelease = qfalse;
		if(ent->client->attackNeedRelease)
			return;
	}
#endif
	...
	if(pm->cmd.buttons & BUTTON_ALT_ATTACK )
	{
		if(pm->ps->weapon == WP_BOWCASTER)
				return;
		#ifdef QAGAME
		else if(pm->ps->weapon == WP_FLECHETTE && g_entities[pm->ps->clientNum].client->skillLevel[SK_FLECHETTE] < FORCE_LEVEL_3)
			return;
	...
	// take an ammo away if not infinite
	if ( pm->ps->clientNum < MAX_CLIENTS && CanDrainAmmo()) //1.3 -- Added CanDrainAmmo, makes this cleaner
	{
		// enough energy to fire this weapon?
...
*/			
	...
	{ //we are a vehicle
		...
		if (veh->m_pVehicle->m_pVehicleInfo->type != VH_FIGHTER && veh->m_pVehicle->m_pVehicleInfo->type != VH_SPEEDER)
...
{//racc - try getting up from a knockdown by using a getup roll move.
	...
	if ( pm->ps->legsAnim == BOTH_KNOCKDOWN1
		||pm->ps->legsAnim == BOTH_DEATH8
		||pm->ps->legsAnim ==BOTH_DEATH6
		||pm->ps->legsAnim ==BOTH_DEATH5
...
{//racc - attempt to get up from a knockdown if we can/need to.
	...
	if ( (legsAnim == BOTH_KNOCKDOWN1
		||legsAnim == BOTH_DEATH8
		||legsAnim == BOTH_DEATH6
		||legsAnim == BOTH_DEATH5
		...
		||legsAnim == BOTH_RELEASED) && (pm->cmd.upmove > 0 || pm->cmd.forwardmove > 0 || pm->cmd.rightmove != 0) ) //[KnockdownChange]
	{//in a knockdown
	...
	else if(legsAnim == BOTH_KNOCKDOWN1
		||legsAnim == BOTH_DEATH8
		||legsAnim == BOTH_DEATH6
		||legsAnim == BOTH_DEATH5
		||legsAnim == BOTH_KNOCKDOWN2
