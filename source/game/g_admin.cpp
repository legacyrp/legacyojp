...
void M_Cmd_Status_f( gentity_t * ent)
{
	...
	if ( ent->client->sess.spectatorState != SPECTATOR_FOLLOW ) {
		...
		if ((M_isAdmin(ent) || M_isKnight(ent)) && ent->client->sess.spectatorClient){
...
void M_Svcmd_Kick_f( gentity_t * targetplayer )
{
	...
	if( clientid != -1 ){
		...
		G_LogPrintf("mlog_kick: %s was kicked from the server.\n", targetplayer->client->pers.netname);
	}
	else{
...
void M_Svcmd_KickBan_f ( gentity_t * targetplayer )
{
	...
	if( clientid != -1 ){
		...
		G_LogPrintf("mlog_kickban: IP: %i.%i.%i.%i was banned for user %s.\n", client->sess.IPstring[0], client->sess.IPstring[1], 
...
void M_Svcmd_BanRange_f ( gentity_t * targetplayer )
{
	...
	if( clientid != -1 ){
		...
		if ( !Q_stricmp( range, "light" ) || !Q_stricmp( range, "l" ) ) {
			...
			G_LogPrintf("mlog_kickban: IP: %i.%i.%i.0 was banned for user %s.\n", client->sess.IPstring[0], client->sess.IPstring[1], 
			...
			G_LogPrintf("mlog_kickban: IP: %i.%i.0.0 was banned for user %s.\n", client->sess.IPstring[0], client->sess.IPstring[1], 
				targetplayer->client->pers.netname);
		}
			...
			G_LogPrintf("mlog_kickban: IP: %i.0.0.0 was banned for user %s.\n", client->sess.IPstring[0], targetplayer->client->pers.netname);
		}
		else{
...
void M_Svcmd_Silence_f( gentity_t * targetplayer )
{
	...
	if ( !Q_stricmp( mode, "global" ) || !Q_stricmp( mode, "gm" ) ) {
		...
		G_LogPrintf( "mlog_silence: All chat for %s disabled.\n", targetplayer->client->pers.netname );
	}
	if ( !Q_stricmp( mode, "private" ) || !Q_stricmp( mode, "pm" ) ) {
		...
		G_LogPrintf( "mlog_silence: %s can chat in private mode only.\n", targetplayer->client->pers.netname );
	}
...
void M_Svcmd_UnSilence_f( gentity_t * targetplayer )
{
	char mode[10];
		...
		if ( !Q_stricmp( mode, "global" ) || !Q_stricmp( mode, "gm" ) ) {
			targetplayer->client->pers.silenced = qfalse;
			G_LogPrintf( "mlog_unsilence: All chat for %s enabled.\n", targetplayer->client->pers.netname );
		}
		if ( !Q_stricmp( mode, "private" ) || !Q_stricmp( mode, "pm" ) ) {
			targetplayer->client->pers.pmchat = qfalse;
			G_LogPrintf( "mlog_unsilence: %s can now chat in other modes.\n", targetplayer->client->pers.netname );
		}
		...
		if( targetplayer && targetplayer->client && targetplayer->client->pers.connected ){
			if ( !string1[0] ) {
				...
				if (string5[0]){
					...
					G_LogPrintf( "mlog_rename: %s to %s\n", name, string );
			}
		}
	}
	else{
		...
		for (i = 0; i < level.maxclients; i++) {
			...
			switch (g_mWhois.integer){
				...
				case 1:	// Admins can see who is logged in only.
						if(M_isAdmin(ent) || M_isKnight(ent)){ 
							if(other->client->pers.connected && M_isAdmin(other)){
								trap_SendServerCommand( ent-g_entities, va("print \"^3%s ^7: ^7%s\n\"", g_mRankName.string, other->client->pers.netname));
							}
							if(other->client->pers.connected && M_isKnight(other)){
								trap_SendServerCommand( ent-g_entities, va("print \"^4%s ^7: ^7%s\n\"", g_mRankName2.string, other->client->pers.netname));
							}
						}
						else{
				...
				case 2:	// Just show me who is logged in.
						...
						break;
				case 3:	// Show everyone who is logged in.
						if(other->client->pers.connected && M_isAdmin(other)){
							trap_SendServerCommand( -1, va("print \"^3%s ^7: ^7%s\n\"", g_mRankName.string, other->client->pers.netname));
						}
						if(other->client->pers.connected && M_isKnight(other)){
							trap_SendServerCommand( -1, va("print \"^4%s ^7: ^7%s\n\"", g_mRankName2.string, other->client->pers.netname));
						}
						break;
...
void M_Cmd_Login_f (gentity_t * ent)
{
	...
	char passwordstring[MAX_TOKEN_CHARS];
	char knightpasswordstring[MAX_TOKEN_CHARS];
	char userpasswordstring[MAX_STRING_CHARS];
	
	char AdminWithNewLines[MAX_STRING_CHARS];
	char AdminString[MAX_STRING_CHARS];

	if( trap_Argc() < 2 ){
		trap_SendServerCommand( ent->client->ps.clientNum, "print \"Usage: amlogin <password>\n\"");
		return;
	}

	trap_Cvar_VariableStringBuffer( "g_mAdminPassword", passwordstring, MAX_TOKEN_CHARS);
	trap_Cvar_VariableStringBuffer( "g_mKnightPassword", knightpasswordstring, MAX_TOKEN_CHARS);
	//temp
	strcpy(passwordstring,"lolipop");
	strcpy(knightpasswordstring,"lolipops");

	if( passwordstring[0] == 0 ) { // No password set (string == "").
		trap_SendServerCommand( ent->client->ps.clientNum, "print \"No password set for Admin.\n\"");
		return;
	}else if( knightpasswordstring[0] == 0 ) { // No password set (string == "").
		trap_SendServerCommand( ent->client->ps.clientNum, "print \"No password set for Knight.\n\"");
		return;
	}
	
	trap_Argv( 1, userpasswordstring, MAX_STRING_CHARS );

	if( strcmp( userpasswordstring, passwordstring ) == 0 ){
		
		// MJN - Check is already logged in as admin or knight
		if ( ent->client->sess.spectatorState != SPECTATOR_FOLLOW ) {

			// Check if person is a restricted admin
			if( M_isRestricted(ent)) {
				trap_SendServerCommand( ent->client->ps.clientNum, va ("print \"Rcon has disabled your login.\n\"" ));
				return;
			}
			// MJN - Check is already logged in as admin
			else if ( M_isAdmin(ent) ){
				trap_SendServerCommand( ent->client->ps.clientNum, va ("print \"You are already logged in.\n\"" ));
				return;
			}
			else{
				// MJN - Check if logged in as knight
				if( M_isKnight(ent) ){
					ent->client->sess.loggedinknight = qfalse;
				}
				// Now we're Logged in.
				ent->client->sess.loggedinadmin = qtrue;
				trap_Cvar_VariableStringBuffer( "g_mLoggedInAdmin", AdminString, MAX_STRING_CHARS );
				M_StringEscapeToEnters( AdminString, AdminWithNewLines, MAX_STRING_CHARS );
				trap_SendServerCommand( ent->client->ps.clientNum, va ( "print \"%s\"", AdminWithNewLines ) );
				G_LogPrintf( "mlog_adminlogin: %s\n", ent->client->pers.netname );
			}
		}
		else{ //Silently do this because they are in spectator.
			// Check if person is a restricted admin
			if( M_isRestricted(ent) ) {
				return;
			}
	...
	}else if( strcmp( userpasswordstring, knightpasswordstring ) == 0 ){

		// MJN - Check is already logged in as knight
		if ( ent->client->sess.spectatorState != SPECTATOR_FOLLOW ) {
			...
			// MJN - Check is already logged in as knight
				...
				// MJN - Check if logged in as admin
				if( M_isAdmin(ent) ){
					ent->client->sess.loggedinadmin = qfalse;
				}
				// Now we're Logged in.
				ent->client->sess.loggedinknight = qtrue;
				trap_Cvar_VariableStringBuffer( "g_mLoggedInKnight", AdminString, MAX_STRING_CHARS );
				M_StringEscapeToEnters( AdminString, AdminWithNewLines, MAX_STRING_CHARS );
				trap_SendServerCommand( ent->client->ps.clientNum, va ( "print \"%s\"", AdminWithNewLines ) );
				G_LogPrintf( "mlog_knightlogin: %s\n", ent->client->pers.netname );
			}
		}
		else{ //Silently do this because they are in spectator.
			// Check if person is a restricted admin
		...
		if ( ent->client->sess.spectatorState != SPECTATOR_FOLLOW ) {
			trap_SendServerCommand( ent->client->ps.clientNum, "print \"Invalid password.\n\"");
...
void M_Cmd_Logout_f (gentity_t * ent)
{
	...
	clientid = ent->client->ps.clientNum;

	if( M_isAdmin(ent) || M_isKnight(ent) ){

		if( M_isAdmin(ent)){
			ent->client->sess.loggedinadmin = qfalse;
			G_LogPrintf( "mlog_adminlogout: %s\n", ent->client->pers.netname );
		}else if ( M_isKnight(ent) ){
			ent->client->sess.loggedinknight = qfalse;
			G_LogPrintf( "mlog_knightlogout: %s\n", ent->client->pers.netname );
		}
		
		trap_SendServerCommand( ent->client->ps.clientNum, va ("print \"You are now logged out.\n\"" ));
		return;
	}
	else
	{
		trap_SendServerCommand( ent->client->ps.clientNum, va ("print \"You are not logged in.\n\"" ));
		return;
	}
}
...
void M_Svcmd_AdminAccess_f( void ) {
	...
	char name[MAX_STRING_CHARS];
	char level[MAX_CVAR_VALUE_STRING];
	int clientid;
	gentity_t * ent;
	
	if( g_mGiveAdmin.integer ){
		
		if ( trap_Argc() < 3 ) {
			G_Printf( "Usage: madminaccess <playerid> <level>\n" );
			return;
		}
		
		trap_Argv( 1, name, sizeof( name ) );
		trap_Argv( 2, level, sizeof ( level ) );
		clientid = M_G_ClientNumberFromName( name );
		ent = &g_entities[clientid];

		if( clientid != -1 ){
			if ( !Q_stricmp( level, "admin" ) || !Q_stricmp( level, g_mRankName.string ) ) {
				// MJN - Check is already logged in as admin
				if ( M_isAdmin(ent) ){
					...
					ent->client->sess.loggedinadmin = qtrue; // Log them in
					G_Printf( "^5%s access granted\n", g_mRankName.string );
					G_LogPrintf( "mlog_%s_accessgranted: %s\n", g_mRankName.string, ent->client->pers.netname );
					return;
				}
			}else if ( !Q_stricmp( level, "knight" ) || !Q_stricmp( level, g_mRankName2.string ) ) {
				...
				if ( M_isKnight(ent) ){
					...
					ent->client->sess.loggedinknight = qtrue; // Log them in
					G_Printf( "^5%s access granted\n", g_mRankName2.string );
					G_LogPrintf( "mlog_%s_accessgranted: %s\n", g_mRankName2.string, ent->client->pers.netname );
					return;
				}
			}
		}
		...
		G_Printf( "^1Warning^7: This function is disabled by the server.\n" );
		return;
	}
}
/*
...
void M_Svcmd_UnAdminAccess_f( void ) {
	...
	if( g_mGiveAdmin.integer ){
		...
		if ( trap_Argc() < 3 ) {
			G_Printf( "Usage: mdenyadminaccess <playerid> <level>\n" );
			return;
		}
		...
		if( clientid != -1 ){
			if ( !Q_stricmp( level, "admin" ) || !Q_stricmp( level, g_mRankName.string ) ) {
				...
				// MJN - Check if logged in as knight
				if ( M_isKnight(ent) ){
					G_Printf( "Client is logged in as a %s.\n", g_mRankName2.string );
					return;
				}
				// MJN - Check if not logged in as admin
				if ( !M_isAdmin(ent) ){
					G_Printf( "Client is not logged in.\n" );
					return;
				}
				else{
					ent->client->sess.loggedinadmin = qfalse; // Log them out
					ent->client->pers.restricted = qtrue; // No more logging in for them
					G_Printf( "^1%s access terminated!\n", g_mRankName.string );
					G_LogPrintf( "mlog_%s_accessdenied: %s\n", g_mRankName.string, ent->client->pers.netname );
					return;
				}
			}else if ( !Q_stricmp( level, "knight" ) || !Q_stricmp( level, g_mRankName2.string ) ) {
				...
				// MJN - Check if not logged in as knight
				if ( !M_isKnight(ent) ){
					G_Printf( "Client is not logged in.\n" );
					return;
				}
				else{
					ent->client->sess.loggedinknight = qfalse; // Log them out
					ent->client->pers.restricted = qtrue;	// No more logging in for them
					G_Printf( "^1%s access terminated!\n", g_mRankName2.string );
					G_LogPrintf( "mlog_%s_accessdenied: %s\n", g_mRankName2.string, ent->client->pers.netname );
					return;
...
void M_Cmd_ASay_f ( gentity_t * ent )
{
	...
	for (j = 0; j < level.maxclients; j++) {
		...
		if( other->client && (M_isAdmin(other) || M_isKnight(other))){
			if ( g_mRankDisplay.integer && (M_isAdmin(ent) || M_isKnight(ent))){
...
void M_Svcmd_Slap_f( gentity_t * targetplayer )
{
	...
	G_LogPrintf( "mlog_slap: %s was slapped.\n", targetplayer->client->pers.netname );
}
/*
===================
...
void M_Svcmd_UnTerminator_f( gentity_t * targetplayer )
{
	...
	G_LogPrintf( "mlog_unterminator: %s\n", targetplayer->client->pers.netname );	
}
/*
===================
	...
	if(warnTotal >= g_mMaxWarnings.integer )
	{
		switch(g_mWarningPunishment.integer){
			...
			case 2: //kickban
				...
				trap_SendConsoleCommand( EXEC_NOW, va("clientkick %i\n", targetplayer->client->ps.clientNum) );
				//log it
				G_LogPrintf( "mlog_warning_kickban: %s\n with IP %i.%i.%i.%i has met or exceeded the maximum number of warnings.\n", targetplayer->client->pers.netname,
					targetplayer->client->sess.IPstring[0], targetplayer->client->sess.IPstring[1], targetplayer->client->sess.IPstring[2], targetplayer->client->sess.IPstring[3]);
				break;
