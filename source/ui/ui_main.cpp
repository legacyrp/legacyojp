...
//[UITweaks]
//we want to be able to select seige in solo mode.
/*
...
//			int oldteam = (int)trap_Cvar_VariableValue("ui_startsiegeteam");
		...
		//[/MOREFORCEOPTIONS]
		//Don't need this stuff anymore with the new menu items.
		/*
		...
		*/
		...
		{//used by menus to load up the approprate menus for the mission selection menus
			//for now, I'm just going to buckshot load stuff at this point.
			//UI_LoadSingleMenuFile("ui/ingameforcestatus.menu");
			//UI_LoadSingleMenuFile("ui/ingameMissionSelect1.menu");
			//UI_LoadSingleMenuFile("ui/ingameMissionSelect2.menu");
			//UI_LoadSingleMenuFile("ui/ingameMissionSelect3.menu");
		}
		//[/CoOp]
		else 
		{
...
static void UI_SiegeClassCnt( const int team )
{
	...
	//[SIEGECVARFIX]
	siege_Cvar_Set("ui_infantry_cnt", va("%d", BG_SiegeCountBaseClass(team,0)));
	siege_Cvar_Set("ui_vanguard_cnt", va("%d", BG_SiegeCountBaseClass(team,1)));
	siege_Cvar_Set("ui_support_cnt", va("%d", BG_SiegeCountBaseClass(team,2)));
...
static int UI_MapCountByGameType(qboolean singlePlayer) {
	...
	//[CoOp]
	/* we want CoOp to show up in the menus too.
...
void UI_SiegeInit(void)
{
	...
	//used to minimize cvar usage
	for(i=0;i<256;i++) 
	{  
		ui_siegeStruct[i] = 0;
	}
	 
	trap_Cvar_Set( "ui_siegeInfo", va("%p",ui_siegeStruct) );
	//[/SIEGECVARFIX]
...
//				UI_LoadNonIngame();
	  ...
	  case UIMENU_CLASSSEL:
		...
		case UIMENU_BRIEFING:
			trap_Cvar_VariableStringBuffer("ui_menubrief", buf, sizeof(buf) );
			trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			if(!Menus_ActivateByName(buf))
			{//menu not loaded.  Try loading a .menu with the same name as this menu.
...
void UI_DrawConnectScreen( qboolean overlay ) {
	...
	if( trap_GetConfigString( CS_SERVERINFO, info, sizeof(info) ) ) {
		...
		Text_PaintCenter(centerPoint, yStart, scale, colorWhite, va(sStringEdTemp, Info_ValueForKey( info, "mapname" )), 0, FONT_MEDIUM);
	}
	...
	if (!Q_stricmp(cstate.servername,"localhost")) {
		...
		strcpy(text, va(sStringEdTemp, cstate.servername));
...
//[SIEGECVARFIX]
vmCvar_t    ui_siegeInfo;
char 		*ui_siegeStruct[1024];
//[/SIEGECVARFIX]
...
static cvarTable_t		cvarTable[] = {
	...
	//[TABBot]
	{ &ui_bottype, "ui_bottype", "1", CVAR_ARCHIVE|CVAR_INTERNAL },
	//[TABBot]
