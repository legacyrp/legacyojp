...
void FX_TuskenRifleMainShot(vec3_t start,vec3_t end)
{
	trap_FX_AddLine( start, end, 0.1f, 6.0f, 0.0f, 
							1.0f, 0.0f, 0.0f,
							WHITE, WHITE, 0.0f,
							150, trap_FX_RegisterEffect( "tusken/shot" ), 
...
void FX_TuskenRifleThink(centity_t *cent, const struct weaponInfo_s *weapon)
{
	...
	trap_FX_PlayEffectID( trap_FX_RegisterEffect("tusken/shot"), cent->lerpOrigin, forward, -1, -1 );
}
...
void FX_TuskenHitWall(vec3_t origin, vec3_t normal)
{
	trap_FX_PlayEffectID( cgs.effects.tuskenWallImpactEffect, origin, normal, -1, -1 );
}
/*
---------------------------
...
void FX_TuskenWeaponHitPlayer( vec3_t origin, vec3_t normal)
{
	trap_FX_PlayEffectID( cgs.effects.tuskenFleshImpactEffect, origin, normal, -1, -1 );
}
