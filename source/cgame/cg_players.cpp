...
static void CG_ForceGripEffect( vec3_t org )
{
		...
		float vLen;
		float alpha=25;
		float tDif = sin( cg.time * 0.004f ) * 0.08f + 0.1f;
...
/*
		...
		memset( &ent, 0, sizeof( ent ) );
		ent.shaderTime = (250-REFRACT_EFFECT_DURATION) / 1000.0f;
	...
	ex->refEntity.renderfx = (RF_DISTORTION|RF_FORCE_ENT_ALPHA);
	ex->refEntity.shaderRGBA[0] = 255.0f;
	ex->refEntity.shaderRGBA[1] = 255.0f;
	ex->refEntity.shaderRGBA[2] = 255.0f;
	ex->refEntity.shaderRGBA[3] = alpha;
	ex->refEntity.customShader = cgs.media.refractionShader;
	ex->refEntity.hModel = cgs.media.explosionModel;
...
{//renders holstered weapons on players.
	...
	{//this player can have holstered weapons
		...
		if(backInUse //back in use already
			|| !(weapInv & (1 << WP_TUSKEN_RIFLE)) //don't have weapon
			|| cent->currentState.weapon == WP_TUSKEN_RIFLE) //currently using weapon
		{//don't render weapon on back
			if(ci->holster_launcher != -1 && ci->launcher_holstered == WP_TUSKEN_RIFLE)
			{
				if(trap_G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), G2MODEL_LAUNCHER_HOLSTERED))
				{
					trap_G2API_RemoveGhoul2Model(&(cent->ghoul2), G2MODEL_LAUNCHER_HOLSTERED);
		...
		{//render weapon on back
			if(ci->holster_launcher != -1)
			{//have specialized bolt
				if(ci->launcher_holstered != WP_TUSKEN_RIFLE)
				{//don't already have the weapon bolted.
					if(ci->launcher_holstered != 0)
					{//we have something else bolted there, remove it first.
						if(trap_G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), G2MODEL_LAUNCHER_HOLSTERED))
					...
					trap_G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_TUSKEN_RIFLE), 0, cent->ghoul2, 
						G2MODEL_LAUNCHER_HOLSTERED);
					trap_G2API_SetBoltInfo(cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED, ci->holster_launcher);
					ci->launcher_holstered = WP_TUSKEN_RIFLE;
				}
			}
			else
			{//manually render the weapon
				CG_HolsteredWeaponRender(cent, ci, HLR_TUSKEN_RIFLE);
			}
			backInUse = qtrue;
		}
...
void CG_Player( centity_t *cent ) {
	...
	else
	{
		if (!cent->npcClient)
		{
			...
			cent->npcClient->gender = (gender_t)FindGender(CG_ConfigString( CS_MODELS+cent->currentState.modelindex ), cent);
			//[/CoOp]
		}
	...
	if ( cent->currentState.number == cg.snap->ps.clientNum) 
	{
		if (!cg.renderingThirdPerson) 
		{
			...
			if ( ( !cg_trueguns.integer && cg.predictedPlayerState.weapon != WP_SABER 
				...
				&& cg.predictedPlayerState.weapon != WP_TUSKEN_RIFLE) 
...
// minimal_add:
	...
	if (cg.snap && cent->currentState.number == cg.snap->ps.clientNum)
	{
		if ( !cg.renderingThirdPerson && (cg_trueguns.integer || cent->currentState.weapon == WP_SABER 
			|| cent->currentState.weapon == WP_MELEE || cent->currentState.weapon == WP_TUSKEN_RIFLE) 
			&& !cg.predictedPlayerState.zoomMode)	
		{
			//<True View varibles
...
SkipTrueView:
	...
	{//doing the electrocuting
		...
		AnglesToAxis( fAng, axis );
		trap_R_AddLightToScene(efOrg, 100, 0.7294f, 0.5255f, 0.9765f );
...
//								assert(0);
	...
	*/
			...
			if (cg.renderingThirdPerson || cent->currentState.number != cg.predictedPlayerState.clientNum)
			{
				...
				*/
				...
				if (cg_shadows.integer != 2 && cgs.glconfig.stencilBits >= 4 && cg_renderToTextureFX.integer)
				{
					...
					if(!VectorCompare(cent->playerState->velocity,vec3_origin))
					trap_R_AddRefEntityToScene( &legs );
					legs.customShader = 0;
				}
				else
	...
	/*if (cent->currentState.forcePowersActive & (1 << FP_MANIPULATE))
