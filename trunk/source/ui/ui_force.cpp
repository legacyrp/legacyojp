...
void UI_UpdateClientForcePowers(const char *teamArg)
{
	...
	if (gTouchedForce)
	{
		...
		else
		{
			...
			//trap_Cmd_ExecuteText( EXEC_APPEND, va("say x %s\n", newForceString) );
...
{//racc - updates the current force powers setup based on current powers selected.
	...
	*/
	...
	if(menu)
	{
		if(uiForceRank >= 50)
		{
			Menu_ShowItemByName(menu, "setsk_detpack", qtrue);
			Menu_ShowItemByName(menu, "setsk_thermal", qtrue);
		}
		else
		{
	...
	{
		if(uiRank[NUM_FORCE_POWERS+NUM_SKILLS+i].uiForcePowersRank)
		{
			setMenuSkill(i,uiRank[NUM_FORCE_POWERS+NUM_SKILLS+i].uiForcePowersRank);
		}
		else
		{
			setMenuSkill(i,0);
	...
	{//Level 0 or 1, hide everything
		uiRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank=0;
		uiRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank=0;
		uiRank[NUM_FORCE_POWERS+SK_CRYOBAN].uiForcePowersRank=0;
		menu = Menus_FindByName("ingame_playergunnery");
		if(menu)
		{
			Menu_ShowItemByName(menu, "setsk_flashgrenade", qfalse);
			Menu_ShowItemByName(menu, "setsk_smokegrenade", qfalse);
			Menu_ShowItemByName(menu, "setsk_cryoban", qfalse);
		}
		if(uiRank[NUM_FORCE_POWERS+SK_GRENADE].uiForcePowersRank == 0)
		{
			if(menu)
				Menu_ShowItemByName(menu, "setsk_empgrenade", qfalse);
		}
		else
			if(menu)
			{
				Menu_ShowItemByName(menu, "setsk_empgrenade", qtrue);
				uiRank[NUM_FORCE_POWERS+SK_EMP].uiForcePowersRank=1;
			}

	}
	else if(uiRank[NUM_FORCE_POWERS+SK_GRENADE].uiForcePowersRank == 2)
	{//level 2, hide cryoban
		uiRank[NUM_FORCE_POWERS+SK_CRYOBAN].uiForcePowersRank=0;
		menu = Menus_FindByName("ingame_playergunnery");
		if(menu)
		{
			Menu_ShowItemByName(menu, "setsk_flashgrenade", qtrue);
			Menu_ShowItemByName(menu, "setsk_smokegrenade", qtrue);
			Menu_ShowItemByName(menu, "setsk_cryoban", qfalse);
		}
	}
	else if(uiRank[NUM_FORCE_POWERS+SK_GRENADE].uiForcePowersRank == 3)
	{
		menu = Menus_FindByName("ingame_playergunnery");
		if(menu)
		{
			Menu_ShowItemByName(menu, "setsk_flashgrenade", qtrue);
			Menu_ShowItemByName(menu, "setsk_smokegrenade", qtrue);
			Menu_ShowItemByName(menu, "setsk_cryoban", qtrue);
		}
	}

	if(uiRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank
		|| uiRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank
		||uiRank[NUM_FORCE_POWERS+SK_CRYOBAN].uiForcePowersRank
		|| uiRank[NUM_FORCE_POWERS+SK_EMP].uiForcePowersRank)
	{
		//uiRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank=0;
		//uiRank[NUM_FORCE_POWERS+SK_CYROBAN].uiForcePowersRank=0;
		//smoke+flash
		//smoke+cyro
		//flash+cyro
		//smoke+emp
		...
		if(uiRank[NUM_FORCE_POWERS+SK_CRYOBAN].uiForcePowersRank
			&& uiRank[NUM_FORCE_POWERS+SK_EMP].uiForcePowersRank)
		{
			if(prevRank && prevRank[NUM_FORCE_POWERS+SK_CRYOBAN].uiForcePowersRank)
				uiRank[NUM_FORCE_POWERS+SK_CRYOBAN].uiForcePowersRank=0;//Just bought emp
			else
				uiRank[NUM_FORCE_POWERS+SK_EMP].uiForcePowersRank=0;//Just bought cryoban
		}

		if(uiRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank
			&& uiRank[NUM_FORCE_POWERS+SK_EMP].uiForcePowersRank)
		{
			if(prevRank && prevRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank)
				uiRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank=0;//Just bought emp
			else
				uiRank[NUM_FORCE_POWERS+SK_EMP].uiForcePowersRank=0;//Just bought flash
		}

		if(uiRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank
			&& uiRank[NUM_FORCE_POWERS+SK_EMP].uiForcePowersRank)
		{
			if(prevRank && prevRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank)
				uiRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank=0;//Just bought emp
			else
				uiRank[NUM_FORCE_POWERS+SK_EMP].uiForcePowersRank=0;//Just bought smoke
		}
		...
		{
			if(prevRank && prevRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank)
				uiRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank=0;//Just bought flash
			else
				uiRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank=0;//Just bought smoke
		}
		...
		{
			if(prevRank && prevRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank)
				uiRank[NUM_FORCE_POWERS+SK_SMOKEGRENADE].uiForcePowersRank=0;//Just bought cryoban
			else
				uiRank[NUM_FORCE_POWERS+SK_CRYOBAN].uiForcePowersRank=0;//Just bought smoke
		}
		...
		{
			if(prevRank && prevRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank)
				uiRank[NUM_FORCE_POWERS+SK_FLASHGRENADE].uiForcePowersRank=0;//Just bought cryoban
			else
	...
	if(spentInForce >= 25)
	{
		...
		if(uiRank[NUM_FORCE_POWERS+SK_GRENADE].uiForcePowersRank > FORCE_LEVEL_1)
	...
	*/
	...
	if(uiMaxRank <= 100 && uiRank[FP_SEE].uiForcePowersRank && (int)(trap_Cvar_VariableValue("ojp_trueBalance")) == 1)
	{
		if(uiMaxRank <= 75)
		{
			...
			uiRank[FP_MINDTRICK].uiForcePowersRank = 0;
...
void UI_ReadLegalForce(void)
{
	...
	if (!BG_LegalizedForcePowers(fcfString, uiMaxRank, (qboolean)ui_freeSaber.integer, forceTeam, atoi( Info_ValueForKey( info, "g_gametype" )), 0))
...
void UI_ForceConfigHandle( int oldindex, int newindex )
{
	...
	BG_LegalizedForcePowers(fcfBuffer, uiMaxRank, (qboolean)ui_freeSaber.integer, forceTeam, atoi( Info_ValueForKey( info, "g_gametype" )), 0);
...
void setMenuSkill(int feat, int level)
{
	...
	{
	case FT_BLASTERS:
		uiRank[NUM_FORCE_POWERS+SK_PISTOL].numRanks = level;
		uiRank[NUM_FORCE_POWERS+SK_BLASTER].numRanks = level;
		return;
	case FT_RIFLES:
		uiRank[NUM_FORCE_POWERS+SK_DISRUPTOR].numRanks = level;
		uiRank[NUM_FORCE_POWERS+SK_TUSKEN_RIFLE].numRanks = level;
		return;
	case FT_HEAVY_WEAPONS:
		uiRank[NUM_FORCE_POWERS+SK_REPEATER].numRanks = level;
		uiRank[NUM_FORCE_POWERS+SK_FLECHETTE].numRanks = level;
		return;
	case FT_SPECIALIST_WEAPONS:
		uiRank[NUM_FORCE_POWERS+SK_BOWCASTER].numRanks = level;
		return;
	case FT_DEMOLITION:
		uiRank[NUM_FORCE_POWERS+SK_DETPACK].numRanks = level;
		uiRank[NUM_FORCE_POWERS+SK_ROCKET].numRanks = level;
		return;
	case FT_EXPLOSIVES:
		{
		uiRank[NUM_FORCE_POWERS+SK_THERMAL].numRanks = level;
		if(level == 1)
		{
		uiRank[NUM_FORCE_POWERS+SK_GRENADE].numRanks = 3;
		}
		else
		{
		uiRank[NUM_FORCE_POWERS+SK_GRENADE].numRanks = level;
		}
		return;
		}
	case FT_ENGINEERING:
		uiRank[NUM_FORCE_POWERS+SK_FORCEFIELD].numRanks = level;
		uiRank[NUM_FORCE_POWERS+SK_SEEKER].numRanks = level;
		uiRank[NUM_FORCE_POWERS+SK_SENTRY].numRanks = level;
		return;
	case FT_SIGHT:
	case FT_BACTA:
		uiRank[NUM_FORCE_POWERS+SK_BACTA].numRanks = level;
		return;
	case FT_FLAMETHROWER:
		uiRank[NUM_FORCE_POWERS+SK_FLAMETHROWER].numRanks = level;
		return;
	case FT_JETPACK:
		uiRank[NUM_FORCE_POWERS+SK_JETPACK].numRanks = level;
		return;
	case FT_CLOAK:
		uiRank[NUM_FORCE_POWERS+SK_CLOAK].numRanks = level;
		return;
	case FT_SHIELDS:
