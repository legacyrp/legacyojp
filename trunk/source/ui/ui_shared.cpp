...
const char *String_Alloc(const char *p) {
	...
	while (str) {
		//[test]
		if(!str->str)
		{//RAFIXME - sanity check, not sure what the heck is the problem.
			break;
		}
		///[/test]
...
void Menu_ItemDisable(menuDef_t *menu, char *name,int disableFlag)
{
	...
	for (j = 0; j < count; j++) 
	{
		...
		if (itemFound != NULL) 
		{
			itemFound->disabled = (qboolean)disableFlag;
...
qboolean Script_Exec(itemDef_t *item, char **args) {
	...
	if (String_Parse(args, &val)) {
		...
		s = (char*)strstr(val, "maptransition");
		if(s)
		{
			s += 14;
			strcpy((char*)val, va("map %s", s) );
		}
		//[/CoOp]
...
//[CoOp]
qboolean Script_PlayVoice(itemDef_t *item, char **args) 
{//this is for playing voices on whatever ghoul2 model is currently playing on the screen
	const char *val;
	if (String_Parse(args, &val)) {
		DC->startLocalSound(DC->registerSound(val), CHAN_VOICE);
	}
	return qtrue;
}
//[/CoOp]
...
commandDef_t commandList[] =
{
  ...
  //[CoOp]
  //added playvoice command
  {"playvoice",	&Script_PlayVoice},			//plays a voice on a ghoul2 model.
  //[/CoOp]
};
...
qboolean Item_OwnerDraw_HandleKey(itemDef_t *item, int key) {
  if (item && DC->ownerDrawHandleKey)
  {
	  ...
	  if( key == A_MOUSE1 || key == A_MOUSE2 )
	  {
		  if(IsForceRank(item->window.ownerDraw) 
			  || item->window.ownerDraw == UI_FORCE_SIDE)
...
keywordHash_t itemParseKeywords[] = {
	...
	//[CoOp]
	//adding cvarSubString support
	{"cvarSubString",	ItemParse_cvarSubString,	NULL	},
	//[/CoOp]
...
static void Item_TextScroll_BuildLines ( itemDef_t* item )
{
	...
	//[CoOp]
	//don't do positional updates if we don't have any text in this item at the moment.
	if( !psText )
	{
		return;
	}
...
uiRank_t uiRank[NUM_TOTAL_SKILLS] = 
{
	...
	{3,UI_FORCE_RANK_LEVITATION,FP_LEVITATION,FORCE_NEUTRAL,qfalse,0},
	{3,UI_FORCE_RANK_SPEED,FP_SPEED,FORCE_NEUTRAL,qfalse,0},
	{3,UI_FORCE_RANK_PUSH,FP_PUSH,FORCE_NEUTRAL,qfalse,0},
	{3,UI_FORCE_RANK_PULL,FP_PULL,FORCE_NEUTRAL,qfalse,0},
	{3,UI_FORCE_RANK_TELEPATHY,FP_MINDTRICK,FORCE_LIGHTSIDE,qfalse,0},
	...
	{3,UI_FORCE_RANK_PROTECT,FP_MANIPULATE,FORCE_LIGHTSIDE,qfalse,0},
	{3,UI_FORCE_RANK_ABSORB,FP_ABSORB,FORCE_DARKSIDE,qfalse,0},
	{3,UI_FORCE_RANK_TEAM_HEAL,FP_TEAM_HEAL,FORCE_LIGHTSIDE,qfalse,0},
	{3,UI_FORCE_RANK_TEAM_FORCE,FP_LIFT,FORCE_DARKSIDE,qfalse,0},
	{3,UI_FORCE_RANK_DRAIN,FP_DRAIN,FORCE_DARKSIDE,qfalse,0},
	{3,UI_FORCE_RANK_SEE,FP_SEE,FORCE_NEUTRAL,qfalse,0},
	...
	{1,UI_FORCE_RANK_BLUESTYLE,NUM_FORCE_POWERS+SK_BLUESTYLE,FORCE_LIGHTSABER,qfalse,0},
	{1,UI_FORCE_RANK_REDSTYLE,NUM_FORCE_POWERS+SK_REDSTYLE,FORCE_LIGHTSABER,qfalse,0},
	{1,UI_FORCE_RANK_PURPLESTYLE,NUM_FORCE_POWERS+SK_PURPLESTYLE,FORCE_LIGHTSABER,qfalse,0},
	{1,UI_FORCE_RANK_GREENSTYLE,NUM_FORCE_POWERS+SK_GREENSTYLE,FORCE_LIGHTSABER,qfalse,0},
	{1,UI_FORCE_RANK_DUALSTYLE,NUM_FORCE_POWERS+SK_DUALSTYLE,FORCE_LIGHTSABER,qfalse,0},
	{1,UI_FORCE_RANK_STAFFSTYLE,NUM_FORCE_POWERS+SK_STAFFSTYLE,FORCE_LIGHTSABER,qfalse,0},
	...
	{3,UI_FORCE_RANK_TUSKEN_RIFLE,NUM_FORCE_POWERS+SK_TUSKEN_RIFLE,0,qfalse,0},
	{3,UI_FORCE_RANK_SHIELD,NUM_FORCE_POWERS+SK_SHIELD,0,qfalse,0},
	//[Grenade]
	{3,UI_FORCE_RANK_GRENADE,NUM_FORCE_POWERS+SK_GRENADE,0,qfalse,0},
	...
	//[/Grenade]
	//Feats
	{3,UI_FORCE_RANK_FEAT_BLASTERS,NUM_FORCE_POWERS+NUM_SKILLS+FT_BLASTERS,0,qfalse,0},
	{3,UI_FORCE_RANK_FEAT_RIFLES,NUM_FORCE_POWERS+NUM_SKILLS+FT_RIFLES,0,qfalse,0},
	{3,UI_FORCE_RANK_FEAT_HEAVY_WEAPONS,NUM_FORCE_POWERS+NUM_SKILLS+FT_HEAVY_WEAPONS,0,qfalse,0},
	{3,UI_FORCE_RANK_FEAT_SPECIALIST_WEAPONS,NUM_FORCE_POWERS+NUM_SKILLS+FT_SPECIALIST_WEAPONS,0,qfalse,0},
	{1,UI_FORCE_RANK_FEAT_DEMOLITION,NUM_FORCE_POWERS+NUM_SKILLS+FT_DEMOLITION,0,qfalse,0},
	{1,UI_FORCE_RANK_FEAT_EXPLOSIVES,NUM_FORCE_POWERS+NUM_SKILLS+FT_EXPLOSIVES,0,qfalse,0},
	{1,UI_FORCE_RANK_FEAT_ENGINEERING,NUM_FORCE_POWERS+NUM_SKILLS+FT_ENGINEERING,0,qfalse,0},
	{2,UI_FORCE_RANK_FEAT_SIGHT,NUM_FORCE_POWERS+NUM_SKILLS+FT_SIGHT,0,qfalse,0},
	{2,UI_FORCE_RANK_FEAT_BACTA,NUM_FORCE_POWERS+NUM_SKILLS+FT_BACTA,0,qfalse,0},
	{1,UI_FORCE_RANK_FEAT_FLAMETHROWER,NUM_FORCE_POWERS+NUM_SKILLS+FT_FLAMETHROWER,0,qfalse,0},
	{1,UI_FORCE_RANK_FEAT_JETPACK,NUM_FORCE_POWERS+NUM_SKILLS+FT_JETPACK,0,qfalse,0},
	{1,UI_FORCE_RANK_FEAT_CLOAK,NUM_FORCE_POWERS+NUM_SKILLS+FT_CLOAK,0,qfalse,0},
	{3,UI_FORCE_RANK_FEAT_SHIELDS,NUM_FORCE_POWERS+NUM_SKILLS+FT_SHIELDS,0,qfalse,0},
	//Add new skills here
};
