...
static void CG_ItemPickup( int itemNum ) {
	...
	if ( cg.snap && bg_itemlist[itemNum].giType == IT_WEAPON ) {
		...
		{ //only autoselect if not explosive ("safe")
			if (bg_itemlist[itemNum].giTag != WP_GRENADE &&
...
extern void CG_AddToAddText(char *text);
	...
	*/
	//[/SaberLockSys]
			...
			case EV_FLASHGRENADE:
				{
					vec3_t up={0,0,1};
					trap_FX_PlayEffectID( trap_FX_RegisterEffect("flash/faceflash"), position, up, -1, -1 );
				}
				break;
			...
			case EV_SMOKEGRENADE:
				{
					vec3_t up={0,0,1};
					trap_FX_PlayEffectID( trap_FX_RegisterEffect("smoke/smokegrenade"), position, up, -1, -1 );
				}
				break;
			...
			case EV_CRYOBAN_EXPLODE:
				{
					vec3_t up={0,0,1};
					trap_FX_PlayEffectID( trap_FX_RegisterEffect("cryoban/explosion"), position, up, -1, -1 );
					//trap_FX_PlayEffectID( trap_FX_RegisterEffect("cryoban/stuck"),cent->playerState->origin, up,-1,-1);
				}
				break;
			...
			case EV_EMPGRENADE:
				{
					vec3_t up={0,0,1};
					trap_FX_PlayEffectID( cgs.effects.mAltDetonate, position, up, -1, -1 );
					//FX_DEMP2_AltDetonate(position,6);
					break;
				}
	...
	case EV_ENTITY_SOUND:
		...
		if(es->trickedentindex == CHAN_VOICE || es->trickedentindex == CHAN_VOICE_GLOBAL)
		{
			...
			{		
				char *r;
				s = CG_ConfigString( CS_SOUNDS + es->eventParm );
				r = (char*)strstr(s, "SOUND/CHARS/JADEN_MALE/");
