...
//[/TicketFix234]
#define MAX_SHIELD_HEIGHT			150//254
#define MAX_SHIELD_HALFWIDTH		150//255
...
//----------------------------------------------------------------
{
	...
	fwd[0]+=Q_irand(0,20);//1.3
	fwd[1]+=Q_irand(0,20);//1.3
	VectorNormalize(fwd);
...
//-----------------------------------------------------
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if(target->s.NPC_class == CLASS_SEEKER)
			continue;
...
void ItemUse_Sentry(gentity_t *ent)
{
	...
	ent->client->ps.hackingBaseTime = 5000;
}
...
void EWeb_SetBoneAnim(gentity_t *eweb, int startFrame, int endFrame)
{
	...
	{ //already playing this anim, let's flag it to restart
		eweb->s.torsoFlip = (qboolean)(!eweb->s.torsoFlip);
	}
	else
	{
...
int Pickup_Ammo (gentity_t *ent, gentity_t *other)
{
	...
	{ //an ammo_all, give them a bit of everything
		if ( g_gametype.integer == GT_SIEGE )	// complaints that siege tech's not giving enough ammo.  Does anything else use ammo all?
		{
			...
			if (other->client->ps.stats[STAT_WEAPONS] & (1<<WP_GRENADE))
			{
...
int Pickup_Weapon (gentity_t *ent, gentity_t *other) {
	...
	if(other->client->ps.fd.forcePowerLevel[FP_SEE] >= FORCE_LEVEL_1) return 0;
